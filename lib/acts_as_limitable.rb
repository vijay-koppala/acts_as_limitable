...
module ActsAsLimitable
  ...
  def self.incr_bucket_val klass, owner_id, method:, at_time: , duration:, amount: 0
    bucket = "#{klass}:#{method}:#{owner_id}:#{duration}:#{(at_time.to_i / duration).to_i}"
  ...
  def self.check_limit klass, owner_id, method:, at_time: , duration:, limit:, amount: 1 
    current_val = incr_bucket_val klass, owner_id, method: method, at_time: at_time, duration: duration, amount: 0
  ...
  def self.incr_bucket_vals klass, owner_id, method:, at_time: Time.now.utc, limits:, amount: 1
    limits.each do |duration, limit|
      response = incr_bucket_val(klass,owner_id, method: method, at_time: Time.now.utc, duration: duration.to_i, 
          amount: amount)
    end
   end

  # Check multiple buckets
  def self.check_limit_multi klass, owner_id, method:, at_time: Time.now.utc, amount: 1,
                            limits: [{1.seconds => 10}, {1.minutes => 120}, {1.hour => 240} ]
    error_message = nil
    limits.each do |duration, limit|
      response = check_limit(klass,owner_id, method: method, at_time: Time.now.utc, duration: duration.to_i, 
